...
class NodeService : Service() {
    ...
    private suspend fun pollTasks(api: String, nodeId: String, token: String) {
        try {
            ...
            val response = get(api + "/api/tasks/poll?nodeId=" + encoded, token)
            if (response.isBlank()) return
            val array = JSONObject(response).optJSONArray("tasks") ?: JSONArray()
            for (i in 0 until array.length()) {
                executeTask(api, token, array.getJSONObject(i))
            }
        } catch (_: Exception) {
            // Keep the node heartbeat loop alive if task polling fails.
        }
    }
    ...
    private suspend fun executeTask(api: String, token: String, task: JSONObject) {
        ...
        try {
            ...
            post(
                api + "/api/tasks/" + URLEncoder.encode(id, "UTF-8"),
                token,
                JSONObject().put("status", "completed")
                    .put("result", JSONObject().put("message", "Task executed on Android")).toString()
            )
        } catch (e: Exception) {
            post(
                api + "/api/tasks/" + URLEncoder.encode(id, "UTF-8"),
                ...
                JSONObject().put("status", "failed")
                    .put("result", JSONObject().put("message", e.message ?: "Execution failed")).toString()
            )
        }
    }
    ...
    private fun openBrowser(target: String) {
        if (target.isBlank() || (!target.startsWith("http://") && !target.startsWith("https://"))) {
            throw IllegalArgumentException("A valid http/https URL is required")
        }
    ...
    private fun openSocialApp(target: String) {
        ...
        val launch = packageManager.getLaunchIntentForPackage(packageName)
            ?: throw IllegalArgumentException("App is not installed")
